...
/**
 * I have to thank you Mr.Tony Dewan that he developed this librarie.
 * @package Laravel
 * @author Mr.Inthra Onsap (Just a Thai boy.)
 * @version 1.0
 */
 ...
 * 3 are required:
    ...
    style_dir
    STRING Path to the style directory.
	
    cache_dir
    STRING Path to the cache directory.  Must be writable.

 * 6 are not required:

    base_uri
    STRING Base uri of the site, like http://www.example.com/ 
    Defaults to the URL::to("/").
    ...
    force_curl
    BOOLEAN Flags whether cURL should always be used for URL file references. 
    Defaults to FALSE.
...
-----------------------------------------------------------------------------------------------
    ...
    Carabiner::js('wymeditor/wymeditor.js', 'wymeditor/wymeditor.pack.js' );

    // add a css file with prebuilt production version
    Carabiner::css('framework/type.css', 'screen', 'framework/type.pack.css');
-----------------------------------------------------------------------------------------------
	
And to prevent an individual asset file from being combined:
-----------------------------------------------------------------------------------------------
    // JS: pass a boolean FALSE as the third attribute of the method
    Carabiner::js('wymeditor/wymeditor.js', 'wymeditor.pack.js', FALSE );

    // CSS: pass a boolean FALSE as the fourth attribute of the method
    Carabiner::css('framework/type.css', 'screen', 'framework/type.pack.css', FALSE);
-----------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------
    ...
    Carabiner::css( array('mobile.css', 'handheld', 'mobile.prod.css') );
    ...
    Carabiner::js( $js_assets );
-----------------------------------------------------------------------------------------------


Carabiner is smart enough to recognize URLs and treat them differently:
-----------------------------------------------------------------------------------------------
    Carabiner::js('http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.js');
-----------------------------------------------------------------------------------------------
...
use Inthraonsap\Carabiner\Exceptions\MissingPathException;
use Inthraonsap\Carabiner\Exceptions\MissingArgumentException;
...
class Carabiner {   
    ...
    * Class Constructor
    */
    public function __construct(Repository $config, URL $url, File $file, Environment $view)
    {
        $this->config = $config;
        $this->url  = $url;
        $this->file = $file;
        ...
        $this->cssMin = new Minify_CSS();
        $this->curl = new Curl();
        
        $carabinerConfig = $this->config->get('carabiner::config');
        $this->config($carabinerConfig);
        $this->bladeCompiler = new BladeCompiler($file, $carabinerConfig['cacheDir']);
    ...
    */	
    public function config($config)
    {
        if(!empty($config['group'])){
            foreach($config['group'] as $group_name => $assets){
                $this->group($group_name, $assets);
            }
        }

        // set the default value for base_uri from the config
        $this->base_uri = (!empty($config['base_uri']))? $config['base_uri'] : $this->url->to("/") . '/';
        $this->charset = (!empty($config['charset']))? $config['charset'] : "UTF-8";
        $this->force_curl = (!empty($config['force_curl']))? $config['force_curl'] : FALSE;
        $this->compiler = (!empty($config['compiler']))? $config['compiler'] : 'php';

        // use the provided values to define the rest of them
        $this->script_path = public_path(). DIRECTORY_SEPARATOR . ltrim($config['scriptDir'], '/');
        $this->validateFolder($this->script_path);
        $this->script_uri = $this->url->to($config['scriptDir']).'/';
        
        $this->style_path = public_path(). DIRECTORY_SEPARATOR .ltrim($config['styleDir'], '/');
        $this->validateFolder( $this->style_path );
        $this->style_uri = $this->url->to($config['styleDir']).'/';

        $this->cache_path = public_path(). DIRECTORY_SEPARATOR . ltrim($config['cacheDir'], '/');
        $this->validateFolder( $this->cache_path , true);
	$this->cache_uri = $this->url->to($config['cacheDir']).'/';
     ...
     * @return string
     */
    private function renderString($str, $data = array())
    {
        if ($this->compiler == 'blade')
        {
            ...
            //conpile string by blade engine
            $str = $this->bladeCompiler->compileString($str);
        }
    ...
    */
    public function compileJs($dev_file, $args = array(), $prod_file = '', $combine = TRUE, $minify = TRUE, $group = 'main'){
        ...
        $ref  = ( $this->isURL($dev_file) ) ? $dev_file : realpath($path.$dev_file);
        $fileContent = $this->_get_contents($ref);
        $fileContent = $this->renderString($fileContent, $args);
        
        $scriptCacheDirectory = ($this->cache_path."script");
        if(!$this->file->isDirectory($scriptCacheDirectory)){
            mkdir($scriptCacheDirectory, 0777, true);
        }
        $filename = md5($fileContent).".js";
        $filePath = $scriptCacheDirectory . DIRECTORY_SEPARATOR . $filename;
        if(!file_exists($filePath)){
            $success = file_put_contents($filePath, $fileContent);
        }
        $cachePath = $this->config->get("carabiner::config.cacheDir", "");
        $cachePath = $cachePath . "script" . DIRECTORY_SEPARATOR;
        $dev_file = $prod_file = $cachePath . $filename;
    ...
    */		
    public function group($group_name, $assets)
    {
        if(!isset($assets['js']) && !isset($assets['css']) ){
            throw new MissingArgumentException("Carabiner Error: The asset group definition named '{$group_name}' does not contain a well formed array.");
            return;
        }
        if( isset($assets['js']) ){
    ...
    */
    ...
    {
        if(empty($ref)){
            throw new MissingPathException("invalid file path in Carabiner."); 
        }
        $abs_ref = ( substr($ref, 0, 2) == '//' ) ? ('http:' . $ref) : $ref;
        if( $this->isURL($abs_ref) && ( ini_get('allow_url_fopen') == 0 || $this->force_curl ) ){
            //$this->_load('curl');
            $contents = $this->curl->simple_get($abs_ref);
        }else{            
    ...
    */		
    public function empty_cache($flag = 'both', $before = 'now')
    {
        ...
        $files = array_diff(scandir($this->cache_path), array('..', '.'));
        ...
        switch($flag){
            ...
            case 'css':
                ...
                $scriptFiles = array_diff(scandir($this->cache_path.DIRECTORY_SEPARATOR."script"), array('..', '.'));
                foreach( $scriptFiles as $file ){
                    $ext = substr( strrchr( $file, '.' ), 1 );
                    if ( ($ext == $flag) && ( filemtime( $this->cache_path . "script" . DIRECTORY_SEPARATOR . $file ) < $before) ) {
                        $success = unlink( $this->cache_path . "script" . DIRECTORY_SEPARATOR . $file );
                    }
            ...
            default:
                ...
                foreach( $scriptFiles as $file ){
                    $ext = substr( strrchr( $file, '.' ), 1 );
                    if ( ($ext == 'js' || $ext == 'css') && ( filemtime( $this->cache_path .'script'.DIRECTORY_SEPARATOR. $file ) < $before) ) {
                        $success = unlink( $this->cache_path . "script" . DIRECTORY_SEPARATOR . $file );
                    }
                }
                break;
    ...
    private function _display_js_string($group='main'){
        ...
        if(!empty($this->_js_string))
        {
            if( !isset($this->_js_string[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
                    throw new Exceptions("Carabiner: The JavaScript string group named '{$group}' does not exist.");
                    return;
    ...
    private function _display_css_string($group = 'main'){
        ...
        if(!empty($this->_css_string))
        {
            ...
            if( !isset($this->css[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
                    ...
                    throw new Exceptions('error', "Carabiner: The CSS string group named '{$group}' does not exist.");
                    return;
    ...
    */
    private function validateFolder( $folder, $writable = false )
    {
        ...
        {
            throw new MissingPathException( 'i can\'t find your javascripts/stylesheet folder! ( ' . $folder . ' ), make sure its created and set correctly' );
        }
